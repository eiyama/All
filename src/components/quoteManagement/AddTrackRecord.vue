<template>
  <div>
    ...
    <div class="followInAdd" style="margin-top:48px">
      <van-field readonly clickable label="跟进结果" :value="followInvalue" placeholder="选择跟进结果" @click="showPickerRes = true" right-icon="arrow" style="margin: 10px 0px" />
      ...
      <div v-show="isTime" class="Followintime">
        <van-field readonly clickable label="下次跟进时间" :value="model.remindTime" placeholder="选择跟进时间" @click="showPickerTime = true" right-icon="arrow" style="margin-top: 10px 0px" />
      ...
      <!-- 跟进内容 -->
      <van-cell-group>
        <van-field v-model="model.content" label="本次跟进内容" type="textarea" placeholder="请输入跟进内容" rows="1" style="margin: 10px 0px" />
      </van-cell-group>
      <!-- 保存按钮 -->
