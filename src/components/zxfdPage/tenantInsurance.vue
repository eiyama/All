<template>
	<div style="background:#F5F5F5;">
            ...
            <div v-show="isMessage" style="left: 0.3rem;margin-top: -0.1rem;" class="inpoTextMsgBox2">
             ...
             <ul class="messageList">
                ...
                <li class="cb pr">投保人证件号 <i>*</i>
                    <input name="applyIdNumber" @keyup="ownerBirth"  @blur="ownerBlur()" type="text" class="insuranceIpt_2" placeholder="请输入证件号"  v-model="upData.owner.IdentifyNumber" >
            	</li>

				 <li v-show="upData.owner.IdentifyType != '' && upData.owner.IdentifyType != 'IdCard' " class="cb pr">出生日期 <i>*</i>
                    <input name="applyIdType" type="hidden" value="01" class="setIptValue" readonly>  
                    <input name="" type="text" class="insuranceIpt" placeholder="请选择"  v-model="upData.owner.birthday" readonly @click="selectBirthday">
            	</li>

                <li  v-show="upData.owner.IdentifyType != '' && upData.owner.IdentifyType != 'IdCard' " class="cb pr">性别 <i>*</i>
					<div class="sexOrdio">
        ...
        <div class="insuranceColumn">
          ...
          <div class="insuranceColumn">
            <div class="insuranceColumnTop">
            	<h3 class="newCTitleBox_2" >选择购买险种</h3>
            </div>
            <ul class="newInsuranceTextList">
                    <li class="cb pr" style="margin-left:0.3rem" > 
                    	<span style="display: flex;align-items: center;"><i class="current" ></i>居家责任保险 </span> 
                         <a class="InTextMsgBtn" href="javascript:void(0)" @click="isCover = true" data-text=''></a>
                        <div class="setQuantityBox">
                        	<p>{{areaAmount}}/元/户/{{month}}个月</p>
                        </div>
                    </li>
					<div v-show="isCover" id="inpoTextMsgBox2" @click="isCover = false"></div>
					<div v-show="isCover" style="left:0.3rem" class="inpoTextMsgBox2">
						<span class="s1" style="left:2.4rem"></span>
						<span class="s2" style="left:2.4rem"></span>
						<div>
                    ...
                    <div class="InsuranceDate" style="border-bottom:1px solid #eee;padding-left: 0;margin-left: 0.3rem;" v-show="tancanTypeIndex.indexOf(0) != -1">
                        <p style="margin-top: 0.3rem;">户型面积</p>
                        <div class="insuranceSetBox" id="setPackage_1">
                            <a @click="setArea(_item,_index)" :class="{current: _index == areaIndex}" v-for="(_item,_index) in areaList" :key="_index">{{_item.name}}</a>
                        </div>
                    </div>
                    <li class="cb pr" style="margin-left:0.3rem;" > 
                    	<span style="display: flex;align-items: center;"><i :class="{current : tancanTypeIndex.indexOf(1) != -1}" @click="setRadio(1)"></i>监护人责任险</span> <a class="InTextMsgBtn" href="javascript:void(0)" @click="isGuardian = true" data-text=''></a>
                        <div class="setQuantityBox">
                        	<p>{{guardianAmount}}/元/人/{{month}}个月</p>
                        </div>
                    </li>
					 <div v-show="isGuardian" id="inpoTextMsgBox2" @click="isGuardian = false"></div>
                                <div v-show="isGuardian" style="left: 0.3rem;" class="inpoTextMsgBox2">
                                    <span class="s1" style="left: 2.4rem;"></span>
                                    <span class="s2" style="left: 2.4rem;"></span>
                                    <div>
                                        <table class="nweInsuranceTab_2">
                    ...
                    <div class="messageList"  v-show="tancanTypeIndex.indexOf(1) != -1">
                       <li class="cb pr">被监护人姓名<i>*</i>
                        <input name="applyEmail" type="text" class="insuranceIpt_2" placeholder="请输入姓名"  v-model="guardianName" >
            	    </li>
                    </div>
                    <li class="cb pr" style="margin-left:0.3rem" > 
                    	<span style="display: flex;align-items: center;"><i :class="{current : tancanTypeIndex.indexOf(2) != -1}" @click="setRadio(2)"></i>电动自行车第三者责任险</span> <a class="InTextMsgBtn" href="javascript:void(0)" @click="isCar = true" data-text=''></a>
                        <div class="setQuantityBox">
                        	<p>{{carAmount}}/元/台/{{month}}个月</p>
                        </div>
                    </li>
					<div v-show="isCar" id="inpoTextMsgBox2" @click="isCar = false"></div>
                                <div v-show="isCar" style="left: 0.3rem;" class="inpoTextMsgBox2">
                    ...
                    <div class="messageList"  v-show="tancanTypeIndex.indexOf(2) != -1">
                       <li  class="cb pr">车架号<i>*</i>
                        <input name="applyEmail" type="text" class="insuranceIpt_2" placeholder="请输入车架号"  v-model="vin" >
            	    </li>
                    </div>
                    <li class="cb pr" style="margin-left:0.3rem" > 
                    	<span style="display: flex;align-items: center;"><i :class="{current : tancanTypeIndex.indexOf(3) != -1}" @click="setRadio(3)"></i>宠物责任险</span> <a class="InTextMsgBtn" href="javascript:void(0)" @click="isPet = true" data-text=''></a>
                        <div class="setQuantityBox">
                        	<p>{{petAmount}}/元/只/{{month}}个月</p>
                        </div>
                    </li>
					 <div v-show="isPet" id="inpoTextMsgBox2" @click="isPet = false"></div>
                                <div v-show="isPet" style="left: 0.3rem;" class="inpoTextMsgBox2">
                                    <span class="s1" style="left: 2.1rem;"></span>
                                    <span class="s2" style="left: 2.1rem;"></span>
                                    <div>
                                        <table class="nweInsuranceTab_2">
                                            <tr>
                                                <th  style="border: 0; width: 1.32rem;">保障内容</th>
                    ...
                    <div class="messageList"  v-show="tancanTypeIndex.indexOf(3) != -1">

                    <li  class="cb pr">宠物类型<i>*</i>
                        <input name="applyEmail" type="text" class="insuranceIpt_2" placeholder="请输入宠物类型"  v-model="PetType" >
            	     </li>

					  <li  style="line-height: 70px;" class="cb pr">与宠物合照<i>*</i>
                    ...
                    <li class="cb pr" style="margin-left:0.3rem" > 
                    	<span style="display: flex;align-items: center;"><i :class="{current : tancanTypeIndex.indexOf(4) != -1}" @click="setRadio(4)"></i>燃气责任险</span> <a class="InTextMsgBtn" href="javascript:void(0)" @click="isGas = true" data-text=''></a>
                        <div class="setQuantityBox">
                        	<p>{{gasAmount}}/元/户/{{month}}个月</p>
                        </div>
                    </li>
					 <div v-show="isGas" id="inpoTextMsgBox2" @click="isGas = false"></div>
                                <div v-show="isGas" style="left: 0.3rem;" class="inpoTextMsgBox2">
                                    <span class="s1" style="left: 2.1rem"></span>
                                    <span class="s2" style="left: 2.1rem;"></span>
                                    <div>
                                        <table class="nweInsuranceTab_2">
                    ...
                    <div class="InsuranceDate" style="border-bottom:1px solid #eee;padding-left: 0;margin-left: 0.3rem;" v-show="tancanTypeIndex.indexOf(4) != -1">
        ...
        <div style="background:#fff">   
            ...
            <div style="padding:.3rem; padding-bottom:.3rem;">
        ...
        <div style="height:1.5rem;">
        	<div class="insuranceBttomBtnBox">
                <div class="cb">
					<div >
						<span><b style="color:#333; font-weight:500;">保险费用: </b><i class="totalAmount"> {{account}}</i> 元/户/{{month}}个月</span>
		...
		<div class="black_box" style="display:none;">
            <div class="base_msg_box">
            	...
            	<div class="base_msg_text" v-if="InsuranceCode == 'PICC'">
                    ...
                    <a v-show="tancanTypeIndex.indexOf(2) != -1" @click="showDszssListBtn " style="font-weight:500;">《电动自行车第三者责任保险条款》</a>
					<a v-show="tancanTypeIndex.indexOf(3) != -1" @click="showDwsyzrBtn " style="font-weight:500;">《动物饲养责任保险条款》</a>
					<a v-show="tancanTypeIndex.indexOf(1) != -1" @click="showJHRListBtn " style="font-weight:500;">《监护人责任保险条款》</a>
					<a v-show="tancanTypeIndex.indexOf(4) != -1" @click="showRQZRListBtn " style="font-weight:500;">《燃气责任保险条款》</a>
  ...
  <script >
   ...
   import tenantFamily from "./tenantFamily.vue";
   import tenantDDZXC from "./tenantDDZXC.vue"; 
   import tenantTY from "./tenantTY.vue";   
...
//    import eleInsurance from './eleInsurance';
   ...
   const typeCodeList = ['BF001','BF002','BF003','BF004','BF005','BF006','BF007','BF008','BF009'];
     var areaAmountList = [
            [26,51,76],
            [50,100,150],
            [99,199,299]
 ...
 export default {
    data(){
        return{
            ...
            typeList: [{name:'居家责任保险',amount:'26/元/户/3个月'},{name:'监护人责任险',amount:'13/元/人/3个月'},{name:'电动自行车第三者责任险',amount:'46/元/台/3个月'},
            {name:'宠物责任险',amount:'6/元/只/3个月'},{name:'燃气责任险',amount:'18/元/户/3个月'}],
            tancanIndex:0,
            sameOwner:false,
            guardianName:'',
            vin:'',
			...
			upDataList:{
					"distributorCode": "",
					"OrderSource":"",
					"productCode": "FDZ001", //string,产品编号
	...
	created(){
				...
				var tenantsData = JSON.parse(sessionStorage.getItem('tenantsData'));
				this.tancanTypeIndex = tenantsData.tancanTypeIndex;
				this.account = tenantsData.account;
				this.areaIndex = tenantsData.areaIndex;
				this.tancanIndex = tenantsData.tancanIndex;
				this.gasIndex = tenantsData.gasIndex;
				if(this.tancanIndex == 0){
					this.month = 3;
				}else if(this.tancanIndex == 1){
					this.month = 6;
				}else if(this.tancanIndex == 2){
					this.month = 12;
				}
				this.areaAmount = areaAmountList[this.tancanIndex][this.areaIndex];
				this.guardianAmount = otherAmountList[2][this.tancanIndex];
				this.petAmount = otherAmountList[1][this.tancanIndex];
				this.carAmount = otherAmountList[0][this.tancanIndex];
				this.gasAmount = gasAmountList[this.tancanIndex][this.gasIndex]
			},
    methods :{
		ownerBlur(e){
			...
			insuredBirthConfirm: function() { // 选择被保人生日日期
				...
				// this.upDataList.endDate = this.getNowFormatDate(this.CommencementDateValue);
		 ...
		 setAmount(i){
             if(i==1){this.guardianAmount_1 = otherAmountList[2][this.tancanIndex];}
             if(i == 2){this.carAmount_1 = otherAmountList[0][this.tancanIndex];}
             if(i == 3){this.petAmount_1 = otherAmountList[1][this.tancanIndex];}
             if(i == 4){this.gasAmount_1 =  gasAmountList[this.tancanIndex][this.gasIndex]};
        },
        setRadio(index){ 
				var i = index;
            if (this.tancanTypeIndex.indexOf(index) != -1) {
				this.tancanTypeIndex.splice(this.tancanTypeIndex.indexOf(index), 1);
						if(i == 0){ //居家责任险
							this.account = this.account *1 - this.areaAmount
							}else if(i==1){ //监护人责任险
        ...
        setGas(item,index){
			 if(index != this.gasIndex){
                ...
                this.gasAmount = gasAmountList[this.tancanIndex][index];
                this.account = this.areaAmount + this.guardianAmount_1 + this.carAmount_1 + this.petAmount_1 + this.gasAmount_1;
            }else{
                return;
            } 
        ...
        setTancan(item,index){
			if(index == this.tancanIndex){
				...
				this.month = index == 0 ? 3 : index == 1 ? 6 : 12  ;
                for(var i = 0 ; i < this.tancanTypeIndex.length; i++){
                    if( i == 0 ){
				...
				if(this.tancanIndex == 0){
					...
					this.carAmount = 46;
					this.petAmount = 6;
					this.guardianAmount = 13;

					if(this.gasIndex == 0){
						this.gasAmount = 21;
					}else if(this.gasIndex == 1){
						this.gasAmount = 18
					}
					if(this.areaIndex == 0){
						this.areaAmount = 26;
		...
		　　var nd = new Date(d.getTime()+24*60*60*1000); //next day
            ...
            var month = newDate.getMonth() + this.month;//获取当前月份的日期
            if (month>11){
		 ...
		 async submitInsurance(){
				...
				this.upDataList.orderDetail.SchemeCode = "";
				this.upDataList.orderDetail.PremiumCode = "";
				for(var i = 0;i < 5;i++){
					if(this.tancanTypeIndex.indexOf(i) != -1){
						if(this.tancanIndex == 0){ //三个月
							
							if(i == 0){ //居家责任险
								this.upDataList.orderDetail.SchemeCode += "FA001"
								if(this.areaIndex == 0){ //投保面积
									this.upDataList.orderDetail.PremiumCode += "BF001"
								}
								if(this.areaIndex == 1){
									this.upDataList.orderDetail.PremiumCode += "BF004"
								}
								if(this.areaIndex == 2){
									this.upDataList.orderDetail.PremiumCode += "BF007"
								}
							}else if(i==1){ //监护人责任险
								if(this.upDataList.orderDetail.SchemeCode.length > 0){
									this.upDataList.orderDetail.SchemeCode += ",FA003"
								}else{
									this.upDataList.orderDetail.SchemeCode += "FA003"
								}
								if(this.upDataList.orderDetail.PremiumCode.length > 0){
									this.upDataList.orderDetail.PremiumCode += ",BF001"	
								}else{
									this.upDataList.orderDetail.PremiumCode += "BF001"
								}
								...
								if(this.upDataList.orderDetail.SchemeCode.length > 0){
									this.upDataList.orderDetail.SchemeCode += ",FA002"
								}else{
									this.upDataList.orderDetail.SchemeCode += "FA002"
								}
								if(this.upDataList.orderDetail.PremiumCode.length > 0){
								...
								if(this.upDataList.orderDetail.SchemeCode.length > 0){
									this.upDataList.orderDetail.SchemeCode += ",FA004"
								}else{
									this.upDataList.orderDetail.SchemeCode += "FA004"
								}
								// this.upDataList.orderDetail.SchemeCode += "FA004,"
						...
						}else if(this.tancanIndex == 1){ //6个月
							
							if(i == 0){ //居家责任险
								this.upDataList.orderDetail.SchemeCode += "FA001"
								if(this.areaIndex == 0){ //投保面积
									this.upDataList.orderDetail.PremiumCode += "BF002"
								}
								if(this.areaIndex == 1){
									this.upDataList.orderDetail.PremiumCode += "BF005"
								}
								if(this.areaIndex == 2){
									this.upDataList.orderDetail.PremiumCode += "BF008"
								}
							}else if(i==1){ //监护人责任险
								if(this.upDataList.orderDetail.SchemeCode.length > 0){
									this.upDataList.orderDetail.SchemeCode += ",FA003"
						...
						} else if(this.tancanIndex == 2){ //12个月
	
							if(i == 0){ //居家责任险
								this.upDataList.orderDetail.SchemeCode += "FA001"
								if(this.areaIndex == 0){ //投保面积
									this.upDataList.orderDetail.PremiumCode += "BF003"
								}
								if(this.areaIndex == 1){
									this.upDataList.orderDetail.PremiumCode += "BF006"
								}
								if(this.areaIndex == 2){
									this.upDataList.orderDetail.PremiumCode += "BF009"
								}
							}else if(i==1){ //监护人责任险
								// this.upDataList.orderDetail.PremiumCode += "BF003,"	
								// this.upDataList.orderDetail.SchemeCode += "FA003,"
				...
				var self = this;
				Indicator.open('下单中...');
				axiosPostAPI('/PropertyProducts/FDZ001/Order',this.upDataList).then(res=>{
				Indicator.close();
				this.canVote = true;
				// if(res){
				// 	 this.canVote = true;
						...
						sessionStorage.setItem('orderNo',res.result.orderNo);
							 ...
							 Indicator.open('出单中...');
								hander = setInterval(function() {
									time -- ;
									if(time > 0){
										axiosGetAPI('/PropertyProducts/FDZ001/Order/' + res.result.orderNo).then(Data=>{
