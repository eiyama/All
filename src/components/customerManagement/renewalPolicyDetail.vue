<template>
  <div class="home" style="background-color:#FFF">
    <mt-header fixed class="def-header cust-head" title="往年保单详情">
      <div slot="left">
        <!-- <mt-button icon="back" @click="closeRenewalDetail()"></mt-button> -->
        <van-icon name="cross" @click="closeRenewalDetail()" />
      </div>
    </mt-header>
    <div class="policy-content">
      <div class="policy-detail">
        <div class="policy-vehicle">
          <div class="policy-hd" :class="{ 'show': shouldShowVehicle}">
            ...
            <table class="policy-table vehicle-info" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <th>承保公司</th>
                <td>{{renewalData.insCorp ? renewalData.insCorp.shortName : ""}}</td>
              </tr>
              <tr>
                <th>品牌型号</th>
              ...
              <tr>
                <th>注册日期</th>
                <td>{{renewalData.car.registerDate.substring(0,10)}}</td>
              </tr>
              <tr>
                <th>是否过户</th>
              ...
              <tr>
                <th>投保车型</th>
                <td>{{renewalData.car.fullPriceModel}}</td>
              </tr>
            </table>
            <table class="policy-table vehicle-info" cellpadding="0" cellspacing="0">
              <colgroup>
              ...
              <tr>
                <th>车主证件号</th>
                <td>{{renewalData.car.owner.idNo.replace(/(\d{5})\d{6}(\d{6})/, "$1******$2")}}</td>
              </tr>
        ...
        <h3 class="policy-title" v-show="renewalData.bzRiskKind">交强险</h3>
        <table class="policy-table sy-table" cellpadding="0" cellspacing="0" v-if="renewalData.bzRiskKind && renewalData.bzRiskKind.beginDate ">
          <tr>
            <th class="gray-th">交强险日期</th>
            <td class="gray-td">{{renewalData.bzRiskKind.beginDate ? renewalData.bzRiskKind.beginDate.substring(0,10) : "--"}} 至 {{renewalData.bzRiskKind ? renewalData.bzRiskKind.suggestRenewalDate.substring(0,10) : "--"}}</td>
          </tr>
        </table>
        <h3 class="policy-title" v-show="renewalData.bsRiskKind && renewalData.bsRiskKind.beginDate">商业险</h3>
        <table class="policy-table sy-table" cellpadding="0" cellspacing="0" v-if="renewalData.bsRiskKind && renewalData.bsRiskKind.beginDate">
          <tr >
            <th class="gray-th">商业险日期</th>
            <td class="gray-td"> {{renewalData.bsRiskKind.beginDate ? renewalData.bsRiskKind.beginDate.substring(0,10) : "--"}} 至 {{renewalData.bsRiskKind ? renewalData.bsRiskKind.suggestRenewalDate.substring(0,10) : "--"}}</td>
          </tr>
          <tr>
            <th class="gray-th">投保险种</th>
            <td>保额(元)</td>
          </tr>
          <tr v-for='item in renewalData.bsRiskKind.items' v-show="renewalData.bsRiskKind && renewalData.bsRiskKind.items && renewalData.bsRiskKind.items.length > 0">
            ...
            <td>
              ...
              <span v-else-if="item.coverage==0.0">承保</span>
...
export default {
  ...
  mounted: function() {
    this.renewalData.car.passengerCapacity = this.renewalData.car.passengerCapacity - 1
  ...
  methods: {
    closeRenewalDetail: function() {
      // window.history.back();
      this.$emit('closeRenewalDetail');
    },
